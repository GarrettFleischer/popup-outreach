...
-- Create saved table for saved form submissions
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_attendees_event_email_unique ON attendees(event_id, email);

-- Create composite unique constraint to prevent duplicate saved submissions per event
CREATE UNIQUE INDEX IF NOT EXISTS idx_saved_event_email_unique ON saved(event_id, email);
